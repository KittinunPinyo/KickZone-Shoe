...
flowchart LR
    ...
    subgraph System ["ระบบเว็บไซต์ร้านค้าออนไลน์สำหรับจัดจำหน่ายรองเท้า"]
        ...
        %% ตรงกลาง (Include / Extend ของลูกค้า)
        I_C5_1(["คำนวณราคารวม"])
        ...
        %% ฝั่งขวา (ฟังก์ชันของแอดมิน)
        A1(["จัดการหมวดหมู่สินค้า"])
        A2(["จัดการสินค้า"])
        A3(["จัดการคำสั่งซื้อ"])
        A4(["จัดการลูกค้า"])
        A5(["จัดการโปรโมชั่น / ส่วนลด"])
        A6(["จัดการการจัดส่ง"])
        A7(["รายงานและสถิติ"])

        %% ตรงกลาง (Include ของแอดมิน)
        I_A2_1(["เพิ่มสินค้า"])
        I_A2_2(["แก้ไขสินค้า"])
        I_A2_3(["ลบสินค้า"])
        I_A3_1(["ตรวจสอบ / ยืนยันคำสั่งซื้อ"])

        %% --- 🛠️ เทคนิคบังคับจัดหน้า (Invisible Links) ---
        %% จับคู่ซ้าย-ขวา เพื่อดันให้กล่อง System กว้างขึ้นและไม่กองเป็นแนวตั้ง
        G1 ~~~ A1
        G2 ~~~ A2
        C1 ~~~ A3
        C5 ~~~ A4
    ...
    %% Include/Extend ลูกค้า (ลูกศรชี้ไปทางขวาเข้าหาตรงกลาง)
    C5 -. "<< include >>" .-> I_C5_1
    C5 -. "<< include >>" .-> I_C5_2
    C6 -. "<< include >>" .-> I_C6_1
    C6 -. "<< include >>" .-> I_C6_2
    ...
    %% Include แอดมิน (ใช้เทคนิคกลับด้านลูกศร เพื่อดึงวงรี Include ให้อยู่ตรงกลาง)
    I_A2_1 <-. "<< include >>" .- A2
    I_A2_2 <-. "<< include >>" .- A2
    I_A2_3 <-. "<< include >>" .- A2
    I_A3_1 <-. "<< include >>" .- A3
